...
Fontinfo ovgloadfont(const int *Points,
...
void ovgunloadfont(VGPath * glyphs, int n) {
	int i;
	for (i = 0; i < n; i++) {
...
VGImage ovgCreateImageFromJpeg(const char *filename) {
	FILE *infile;
...
void ovgmakeimage(VGfloat x, VGfloat y, int w, int h, VGubyte * data) {
...
void ovgImage(VGfloat x, VGfloat y, int w, int h, const char *filename) {
	VGImage img = ovgCreateImageFromJpeg(filename);
	vgSetPixels(x, y, img, 0, 0, w, h);
	vgDestroyImage(img);
}
...
void ovgInitWindowSize(int x, int y, unsigned int w, unsigned int h) {
	init_x = x;
	init_y = y;
	init_w = w;
	init_h = h;
...
void ovginit(int *w, int *h,DISPMANX_CALLBACK_FUNC_T cb_func) {
	bcm_host_init();
	...
	oglinit(state,cb_func);
	SansTypeface = ovgloadfont(DejaVuSans_glyphPoints,
	...
	SerifTypeface = ovgloadfont(DejaVuSerif_glyphPoints,
	...
	MonoTypeface = ovgloadfont(DejaVuSansMono_glyphPoints,
...
// finish cleans up
void ovgfinish() {
	ovgunloadfont(SansTypeface.Glyphs, SansTypeface.Count);
	ovgunloadfont(SerifTypeface.Glyphs, SerifTypeface.Count);
	ovgunloadfont(MonoTypeface.Glyphs, MonoTypeface.Count);
...
void ovgTranslate(VGfloat x, VGfloat y) {
	vgTranslate(x, y);
}

// Rotate around angle r
void ovgRotate(VGfloat r) {
	vgRotate(r);
}

// Shear shears the x coordinate by x degrees, the y coordinate by y degrees
void ovgShear(VGfloat x, VGfloat y) {
	vgShear(x, y);
}

// Scale scales by  x, y
void ovgScale(VGfloat x, VGfloat y) {
	vgScale(x, y);
}
...
void ovgsetfill(VGfloat color[4]) {
...
void ovgsetstroke(VGfloat color[4]) {
...
void ovgStrokeWidth(VGfloat width) {
...
void ovgRGBA(unsigned int r, unsigned int g, unsigned int b, VGfloat a, VGfloat color[4]) {
	if (r > 255) {
		r = 0;
	}
	if (g > 255) {
...
void ovgRGB(unsigned int r, unsigned int g, unsigned int b, VGfloat color[4]) {
	ovgRGBA(r, g, b, 1.0f, color);
}

// Stroke sets the stroke color, defined as a RGB triple.
void ovgStroke(unsigned int r, unsigned int g, unsigned int b, VGfloat a) {
	VGfloat color[4];
	ovgRGBA(r, g, b, a, color);
	ovgsetstroke(color);
}

// Fill sets the fillcolor, defined as a RGBA quad.
void ovgFill(unsigned int r, unsigned int g, unsigned int b, VGfloat a) {
	VGfloat color[4];
	ovgRGBA(r, g, b, a, color);
	ovgsetfill(color);
}
...
void ovgFillLinearGradient(VGfloat x1, VGfloat y1, VGfloat x2, VGfloat y2, VGfloat * stops, int ns) {
...
void ovgFillRadialGradient(VGfloat cx, VGfloat cy, VGfloat fx, VGfloat fy, VGfloat radius, VGfloat * stops, int ns) {
...
void ovgClipRect(VGint x, VGint y, VGint w, VGint h) {
...
// derived from http://web.archive.org/web/20070808195131/http://developer.hybrid.fi/font2openvg/renderFont.cpp.txt
void ovgText(VGfloat x, VGfloat y, const char *s, Fontinfo f, int pointsize) {
...
VGfloat ovgTextWidth(const char *s, Fontinfo f, int pointsize) {
	VGfloat tw = 0.0;
...
void ovgTextMid(VGfloat x, VGfloat y, const char *s, Fontinfo f, int pointsize) {
	VGfloat tw = ovgTextWidth(s, f, pointsize);
	ovgText(x - (tw / 2.0), y, s, f, pointsize);
}

// TextEnd draws text, with its end aligned to (x,y)
void ovgTextEnd(VGfloat x, VGfloat y, const char *s, Fontinfo f, int pointsize) {
	VGfloat tw = ovgTextWidth(s, f, pointsize);
	ovgText(x - tw, y, s, f, pointsize);
}

// TextHeight reports a font's height
VGfloat ovgTextHeight(Fontinfo f, int pointsize) {
	return (f.font_height * pointsize) / 65536;
}

// TextDepth reports a font's depth (how far under the baseline it goes)
VGfloat ovgTextDepth(Fontinfo f, int pointsize) {
...
void ovgCbezier(VGfloat sx, VGfloat sy, VGfloat cx, VGfloat cy, VGfloat px, VGfloat py, VGfloat ex, VGfloat ey) {
...
void ovgQbezier(VGfloat sx, VGfloat sy, VGfloat cx, VGfloat cy, VGfloat ex, VGfloat ey) {
...
void ovgLine(VGfloat x1, VGfloat y1, VGfloat x2, VGfloat y2) {
	VGPath path = newpath();
...
void ovgRoundrect(VGfloat x, VGfloat y, VGfloat w, VGfloat h, VGfloat rw, VGfloat rh) {
	VGPath path = newpath();
...
void ovgEllipse(VGfloat x, VGfloat y, VGfloat w, VGfloat h) {
	VGPath path = newpath();
...
void ovgCircle(VGfloat x, VGfloat y, VGfloat r) {
	ovgEllipse(x, y, r, r);
}

// Arc makes an elliptical arc at the specified location and dimensions
void ovgArc(VGfloat x, VGfloat y, VGfloat w, VGfloat h, VGfloat sa, VGfloat aext) {
	VGPath path = newpath();
...
void ovgStart(int width, int height) {
...
void ovgSaveEnd(const char *filename) {
	FILE *fp;
	assert(vgGetError() == VG_NO_ERROR);
...
void ovgBackground(unsigned int r, unsigned int g, unsigned int b) {
	VGfloat colour[4];
	ovgRGB(r, g, b, colour);
...
void ovgBackgroundRGB(unsigned int r, unsigned int g, unsigned int b, VGfloat a) {
	VGfloat colour[4];
	ovgRGBA(r, g, b, a, colour);
	vgSetfv(VG_CLEAR_COLOR, 4, colour);
	vgClear(0, 0, state->window_width, state->window_height);
}
...
// transformations)
void ovgAreaClear(unsigned int x, unsigned int y, unsigned int w, unsigned int h) {
	vgClear(x, y, w, h);
}

// WindowOpacity sets the  window opacity
void ovgWindowOpacity(unsigned int a) {
	dispmanChangeWindowOpacity(state, a);
}

// WindowPosition moves the window to given position
void ovgWindowPosition(int x, int y) {
...
void ovgCbezierOutline(VGfloat sx, VGfloat sy, VGfloat cx, VGfloat cy, VGfloat px, VGfloat py, VGfloat ex, VGfloat ey) {
	VGubyte segments[] = { VG_MOVE_TO_ABS, VG_CUBIC_TO };
	VGfloat coords[] = { sx, sy, cx, cy, px, py, ex, ey };
...
void ovgQbezierOutline(VGfloat sx, VGfloat sy, VGfloat cx, VGfloat cy, VGfloat ex, VGfloat ey) {
	VGubyte segments[] = { VG_MOVE_TO_ABS, VG_QUAD_TO };
	VGfloat coords[] = { sx, sy, cx, cy, ex, ey };
	makecurve(segments, coords, VG_STROKE_PATH);
}
...
void ovgRectOutline(VGfloat x, VGfloat y, VGfloat w, VGfloat h) {
	VGPath path = newpath();
	vguRect(path, x, y, w, h);
	vgDrawPath(path, VG_STROKE_PATH);
	vgDestroyPath(path);
...
void ovgRoundrectOutline(VGfloat x, VGfloat y, VGfloat w, VGfloat h, VGfloat rw, VGfloat rh) {
	VGPath path = newpath();
	vguRoundRect(path, x, y, w, h, rw, rh);
	vgDrawPath(path, VG_STROKE_PATH);
	vgDestroyPath(path);
...
void ovgEllipseOutline(VGfloat x, VGfloat y, VGfloat w, VGfloat h) {
	VGPath path = newpath();
	vguEllipse(path, x, y, w, h);
	vgDrawPath(path, VG_STROKE_PATH);
	vgDestroyPath(path);
...
void ovgCircleOutline(VGfloat x, VGfloat y, VGfloat r) {
	ovgEllipseOutline(x, y, r, r);
}

// ArcOutline makes an elliptical arc at the specified location and dimensions, outlined
void ovgArcOutline(VGfloat x, VGfloat y, VGfloat w, VGfloat h, VGfloat sa, VGfloat aext) {
	VGPath path = newpath();
	vguArc(path, x, y, w, h, sa, aext, VGU_ARC_OPEN);
	vgDrawPath(path, VG_STROKE_PATH);
	vgDestroyPath(path);
